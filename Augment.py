...
def add_gaussian_noise(img, mean, var, alpha):
    row, col, _ = img.shape
    ...
    gaussian = np.random.normal(mu, sigma, (row, col, 1)).astype(np.float32)
    gaussian = np.concatenate((gaussian, gaussian, gaussian), axis = 2)
    gaussian_img = cv2.addWeighted(img, alpha, 0.25 * gaussian, beta, 0).astype(np.float32)
    return gaussian_img
  
def transform(image, transform, interpolation = 'nearest', fill_mode = 'constant', fill_value = 50):
